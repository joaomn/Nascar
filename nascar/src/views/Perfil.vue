<template>
  <div
    id="header"
    class="flex flex-wrap align-items-center justify-content-between gap-5 mb-3 p-3"
  >
    ...
    <div>
      <Button
        ...
        @click="updateDisplayDialog = true"
      />
    </div>
  </div>
  <!-- tabela de displays-->
    ...
    >
      <template #header class="gap-5">
        ...
        >
          ...
          <Button
            label="Adicionar Display"
            icon="pi pi-plus"
            severity="success"
            raised
            style="padding: 5px; gap: 5px"
            @click="addDisplayModal = true"
          />
        </div>
      </template>
      <Column
      ...
      <template #footer>
        <div
          class="flex flex-wrap align-items-center justify-content-center p-3"
        >
  ...
  <Dialog
    v-model:visible="addDisplayModal"
    :style="{ width: '650px' }"
    header="Adicionar Display"
    :modal="true"
    class="p-fluid"
  >
    <template #header>
    ...
    <template #footer>
      <Button
        label="Sair"
        icon="pi pi-times"
        @click="addDisplayModal = false"
        autofocus
        class="p-button-text modal-sair-salvar"
      />
      <Button
        label="Salvar"
        icon="pi pi-check"
        @click="addDisplay(), (addDisplayModal = false)"
        class="modal-sair-salvar"
      />
    </template>
  </Dialog>
  ...
  <Dialog
    ...
    :modal="true"
  >
    <div class="textodelete">Tem Certeza que deseja excluir?</div>
    <div class="botaoDeletar">
      <Button
        label="Sair"
        icon="pi pi-times"
        style="background-color: #f2f2f2; width: 120px; height: 45px"
  ...
  <Dialog
    v-model:visible="updateDisplayDialog"
    :style="{ width: '450px' }"
    header="Editar Cadastro"
    :modal="true"
    class="p-fluid titulo-modal"
  >
    <div class="field">
      <div class="p-inputgroup">
        <span class="p-inputgroup-addon">
    ...
    <div class="field">
      <div class="p-inputgroup">
        ...
        <Password v-model="userUpdate.password" toggleMask class="icon-senha" />
      </div>
    </div>
    <template #footer>
      <Button
        label="Sair"
        icon="pi pi-times"
        @click="updateDisplayDialog = false"
        autofocus
        class="p-button-text modal-sair-salvar"
      />
      <Button
        label="Salvar"
        icon="pi pi-check"
        @click="updateUser(sessionUser.id), (updateDisplayDialog = false)"
        class="modal-sair-salvar"
      />
    </template>
  </Dialog>
  ...
  <Dialog
    v-model:visible="updateDisplayModal"
    :style="{ width: '650px' }"
    header="Adicionar Display"
    :modal="true"
    class="p-fluid"
  >
    ...
    <div class="field">
      <div class="p-inputgroup">
        ...
        <InputText
          ...
          v-model="updateDisplayModel.description"
        />
      </div>
    </div>
    <div class="field">
      <div class="p-inputgroup">
        ...
        <ToggleButton
          v-model="updateDisplayModel.active"
          onLabel="Ativo"
          offLabel="Inativo"
          onIcon="pi pi-check"
    ...
    <template #footer>
      <Button
        label="Sair"
        icon="pi pi-times"
        @click="updateDisplayModal = false"
        autofocus
        class="p-button-text modal-sair-salvar"
      />
      <Button
        ...
        icon="pi pi-check"
        @click="
          updateDisplay(updateDisplayModel.id), (updateDisplayModal = false)
        "
        class="modal-sair-salvar"
      />
    </template>
      ...
      await axios
        ...
        .then((resp) => {
          ...
          this.$toast.add({
            ...
            detail: "Display Adicionado com  Sucesso!",
            life: 5000,
          });
          this.getDisplays();
        })
        .catch((err) => {
          ...
          this.$toast.add({
            ...
            detail: "Erro ao Tentar Salvar Display",
            life: 5000,
          });
        });
    },
      ...
      await axios
        ...
        .then((resp) => {
          this.$toast.add({
            severity: "secondary",
            summary: "Sucesso",
            detail: "Display Deletado com Sucesso",
            life: 8000,
          });
          this.getDisplays();
        })
        .catch((err) => {
          this.$toast.add({
            severity: "error",
            summary: "Erro",
            detail: "Erro ao deletar display",
            life: 8000,
          });
        });
    },
      ...
      await axios
        ...
        .then((resp) => {
          this.$toast.add({
            severity: "success",
            summary: "Success Message",
            detail: "Display Atualizado com sucesso!",
            life: 8000,
          });
          this.getDisplays();
        })
        .catch((err) => {
          this.$toast.add({
            severity: "error",
            summary: "Erro",
            detail: "Erro ao Atualizar Display",
            life: 8000,
          });
        });
    },
